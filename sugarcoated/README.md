...
1. **Offline pipeline** (`build_dataset.py`) — run by a developer when the CSVs change. It reads `data/packaged_foods_india.csv` and `data/redflag_ingredients.csv`, calls the same matching and scoring modules the app uses, and writes `data/products.json`. That file is the catalog the UI searches and the pool for alternatives. Precomputing 852 products keeps the app snappy and keeps matching deterministic for the catalog.
2. **Runtime app** (`app.py` + shared modules) — Streamlit only. No Flask, no browser localStorage, no live network calls. Lookups read local JSON/CSV. When you search a product or paste ingredients, matching and scoring run once, the result is cached in Streamlit session state, and the three tabs read that cache.
...
| File | Owns |
| --- | --- |
| `matching.py` | The only ingredient matcher: comma tokens, word-boundary exact match, then RapidFuzz, then overlap cleanup. |
| `scoring.py` | Severity points, raw score, Low/Moderate/High buckets, 0–10 healthscore. |
| `build_dataset.py` | One-shot CSV → `data/products.json`, plus a printed risk-level summary. |
| `alternates.py` | One generic same-`sub_category` top-N finder used by every tab. |
| `macro_insights.py` | Mifflin-St Jeor targets, % of daily limit, protein/fat efficiency, efficiency alternatives. |
| `diet_profiles.py` | Vegan / gluten-free / diabetic-friendly / low-sodium checks with reasons. |
| `app.py` | Streamlit UI: intake, search vs paste, three tabs. |
| `data/packaged_foods_india.csv` | Source product + nutrition table (untouched by the app). |
| `data/redflag_ingredients.csv` | Source red-flag dictionary (untouched by the app). |
| `data/products.json` | Generated catalog. Regenerate; do not hand-edit as source of truth. |
